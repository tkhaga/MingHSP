...
/* $Id: soundstream.c,v 1.24 2005/03/29 14:06:07 whamann Exp $ */
...
int
nextMP3Frame(SWFInput input, int flgGetSamplesNum);
...
SWFSoundStream_getStreamBlock(SWFSoundStream stream)
{
	...
	{
		++block->numFrames;
		length = nextMP3Frame(stream->input, 0);
...
{	int n, frameSize;
	...
	if ( stream->sampleRate > 32000 )
		...
		frameSize = 576;
	n = 0;
	while(nextMP3Frame(stream->input, 0) > 0)
		n++;
	SWFSoundStream_rewind(stream);
